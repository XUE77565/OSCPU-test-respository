...
module id_stage(
  ...
  // 运算器功能选择信号, RISC-V的简便之处在于所有的功能选择都在指令之中体现
  ...
  // 移位器操作数B选择信号
  assign Shiftop_B = (opcode == `I_type_c) ? shamt : rdata2_true[4:0];  // 可变目标左右移量为rs[4:0]
  ...
  assign PC_Jump   = (opcode == `JAL) ? PC_ID + J_imm : Jump_r;
