...
module mem_stage(
  ...
  assign RF_wdata = (opcode == `CSR_type) ? csr_read_data :
                    ...
                    (opcode == `LUI) ? U_imm :
                    (opcode == `I_type_l) ?
                      // 按字节/半字加载, 根据func区分符号扩展还是零扩展
                      ((func[1:0] == 2'b00) ?  // LB/LBU: 单字节
                       ...
                       (func[1:0] == 2'b01) ?  // LH/LHU: 半字
                         ((eff == 2'b00) ? ((func[2] == 0) ? {{16{read_byte_1[7]}}, Read_data_current[15:0]}  : {{16{1'b0}}, Read_data_current[15:0]}) :
