...
module ex_stage(
  ...
  // 复用这套逻辑来进行mret/ecall的跳转
  wire        exec_redirect = EX_work && (inst_mret || inst_ecall);
  wire [31:0] exec_PC       = inst_mret ? mepc_value : mtvec_value;  // mret跳到mepc, ecall跳到mtvec

  assign prediction_incorrect = ((Branch_real ^ prediction_result) || exec_redirect) && ~block_cancel;
